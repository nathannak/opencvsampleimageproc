...
class AsyncServiceHelper
{
    ...
    protected static void InstallService(final Context AppContext, final LoaderCallbackInterface Callback)
    {
        ...
        {
                Log.v(TAG, "Request new service installation");
                ...
                public void install() {
                    Log.v(TAG, "Trying to install OpenCV Manager via Google Play");

                    boolean result = InstallServiceQuiet(AppContext);
                    if (result)
                    {
                        mServiceInstallationProgress = true;
                        Log.v(TAG, "Package installation started");
                    }
                    else
                    {
                        Log.v(TAG, "OpenCV package was not installed!");
                        int Status = LoaderCallbackInterface.MARKET_ERROR;
                        Log.v(TAG, "Init finished with status " + Status);
                        Log.v(TAG, "Unbind from service");
                        Log.v(TAG, "Calling using callback");
                ...
                public void cancel()
                {
                    Log.v(TAG, "OpenCV library installation was canceled");
        ...
        {
            Log.v(TAG, "Waiting current installation process");
            InstallCallbackInterface WaitQuery = new InstallCallbackInterface() {
                ...
                public void cancel()
                {
                    Log.v(TAG, "Wating for OpenCV canceled by user");
    ...
    protected ServiceConnection mServiceConnection = new ServiceConnection()
    {
        public void onServiceConnected(ComponentName className, IBinder service)
        {
            ...
            {
                Log.v(TAG, "OpenCV Manager Service connection fails. May be service was not installed?");
            ...
            else
            {
                ...
                try
                {
                    ...
                    {
                        Log.v(TAG, "Init finished with status " + LoaderCallbackInterface.INCOMPATIBLE_MANAGER_VERSION);
                        Log.v(TAG, "Unbind from service");
                    ...
                    Log.v(TAG, "Trying to get library path");
                    ...
                    if ((null == path) || (path.length() == 0))
                    {
                        if (!mLibraryInstallationProgress)
                        {
                            InstallCallbackInterface InstallQuery = new InstallCallbackInterface() {
                                ...
                                public void install() {
                                    Log.v(TAG, "Trying to install OpenCV lib via Google Play");
                                    try
                                    {
                                        if (mEngineService.installVersion(mOpenCVersion))
                                        {
                                            mLibraryInstallationProgress = true;
                                            Log.v(TAG, "Package installation statred");
                                            Log.v(TAG, "Unbind from service");
                                            mAppContext.unbindService(mServiceConnection);
                                        }
                                        else
                                        {
                                            Log.v(TAG, "OpenCV package was not installed!");
                                            Log.v(TAG, "Init finished with status " + LoaderCallbackInterface.MARKET_ERROR);
                                            Log.v(TAG, "Unbind from service");
                                            mAppContext.unbindService(mServiceConnection);
                                            Log.v(TAG, "Calling using callback");
                                        ...
                                        e.printStackTrace();;
                                        Log.v(TAG, "Init finished with status " + LoaderCallbackInterface.INIT_FAILED);
                                        Log.v(TAG, "Unbind from service");
                                        mAppContext.unbindService(mServiceConnection);
                                        Log.v(TAG, "Calling using callback");
                                ...
                                public void cancel() {
                                    Log.v(TAG, "OpenCV library installation was canceled");
                                    Log.v(TAG, "Init finished with status " + LoaderCallbackInterface.INSTALL_CANCELED);
                                    Log.v(TAG, "Unbind from service");
                                    mAppContext.unbindService(mServiceConnection);
                                    Log.v(TAG, "Calling using callback");
                        ...
                        else
                        {
                            InstallCallbackInterface WaitQuery = new InstallCallbackInterface() {
                                ...
                                public void wait_install() {
                                    Log.v(TAG, "Waiting for current installation");
                                    try
                                    {
                                        if (!mEngineService.installVersion(mOpenCVersion))
                                        {
                                        ...
                                        {
                                            Log.v(TAG, "Wating for package installation");
                                        }
    ...
    private boolean loadLibrary(String AbsPath)
    {
        ...
        Log.v(TAG, "Trying to load library " + AbsPath);
        try
        {
            System.load(AbsPath);
            Log.v(TAG, "OpenCV libs init was ok!");
        }
        catch(UnsatisfiedLinkError e)
        {
            Log.v(TAG, "Cannot load library \"" + AbsPath + "\"");
            e.printStackTrace();
            result &= false;
        }
    ...
    private boolean initOpenCVLibs(String Path, String Libs)
    {
        ...
        {
            Log.v(TAG, "Library path \"" + Path + "\" is empty");
            return false;
        }
    }
}
